	...
	protected KPartiteGraphView<PersistentObject, Partition> execute(
		...
		for (int p = 0; p < partitions.size() - 1; p++) {
			...
			for (PersistentObject node : net.getNodes(partition)) {
				...
				org.knime.knip.tracking.data.graph.TrackedNode myNode = new org.knime.knip.tracking.data.graph.TrackedNode(
						net, node);
				for (PersistentObject nextNode : net.getNodes(nextPartition)) {
					...
					org.knime.knip.tracking.data.graph.TrackedNode myNextNode = new org.knime.knip.tracking.data.graph.TrackedNode(
							net, nextNode);
